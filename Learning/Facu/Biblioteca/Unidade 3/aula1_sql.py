...
# Cahamdo 'exemplo.db' - Se o banco de dados não existir, ele será criado automaticamente

conn = sqlite3.connect('exemplo.db')

# 2. Criar um obejeto cursor
# O Curosr é usado para executar comandos SQL no banco de dados.
...
# Esrta tabela terá quatro colunas: id (chave primária), nome (texto), preco (número real) e estoque (número inteiro).
...
# Fianalmente, você usa o método close no objeto de conexão para encerrar a conexão com o banco de dados. 
...
cursor.execute(novo_produto, inserir_produto)
